...
class LSTMModel():
	def build_model(self, num_vocab, embedding_matrix, max_len):
		...
		l2x = LSTM(512,return_sequences=True)(l1x)
		loutx = TimeDistributed(Dense(EMBEDDING_LEN))(l2x)
...
def main(params):
	...
	model.fit_generator(epochs=8, generator=training_generator, validation_data=validation_generator, steps_per_epoch=(len(Ds.data_train.imgid)/100), validation_steps = len(Ds.data_val.imgid)/125, verbose=1)
	...
	fid = open(datapath + "second_train_pred.txt", "w")
	for p in pred:
		fid.write(str(p[0])+"\n")
	fid.close()
	...
	fid = open(datapath + "second_test_pred.txt", "w")
	for p in pred:
		fid.write(str(p[0])+"\n")
	fid.close()
