...
class LSTMModel():
	def build_model(self, num_vocab, embedding_matrix, max_len):
		...
		l1x = Embedding(input_dim=num_vocab, output_dim=EMBEDDING_LEN, weights=[embedding_matrix], input_length=max_len)(lang_input)
		loutx = Reshape((3000,), input_shape=(10,300))(l1x)

		img_input = Input(shape=(4096,))
		ioutx = Dense(EMBEDDING_LEN, input_dim = 4096, activation='relu')(img_input)

		x = Concatenate(axis=1)([ioutx, loutx])
		x = Dense(5000, activation='relu', W_regularizer=l2(0.0001), b_regularizer=l2(0.0001))(x)
		x = Dense(500, activation='relu', W_regularizer=l2(0.0001), b_regularizer=l2(0.0001))(x)
...
def main(params):
	...
	model.fit_generator(epochs=15, generator=training_generator, validation_data=validation_generator, steps_per_epoch=(len(Ds.data_train.imgid)/100), validation_steps = len(Ds.data_val.imgid)/125, verbose=1)
